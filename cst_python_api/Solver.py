...
class Solver:
    ...
    async def setFrequencyRange(
        ...
        for key, value in freqParams.items():
            ...
            try:
                freqParams[key] = await self.__CheckParam.doCheck(value)
    ...
    async def changeSolverType(self, type: str):
    ...
    async def setBoundaryCondition(
        ...
        try:
            wallCond = await self.__CheckParam.doCheck(wallCond)
        # If an exception occurs, handle it    
        except TypeError:
    ...
    async def addSymmetryPlane(self, xSym: str, ySym: str, zSym: str):
    ...
    async def addFieldMonitor(self, type: str, freq: Union[float, str]):
    ...
    async def setBackgroundMaterial(
        ...
        for key, value in bkgrParams.items():
            # Pass it through the checker
            try:
                bkgrParams[key] = await self.__CheckParam.doCheck(value)
            # If an exception occurs, handle it    
            except TypeError:
                raise TypeError(f"ERROR: {key} must be of type float or str.")
            except RuntimeError:
    ...
    async def defineFloquetModes(
        ...
        for key, value in portParams.items():
            # Pass it through the checker
            try:
                portParams[key] = await self.__CheckParam.doCheck(value)
            # If an exception occurs, handle it    
            except TypeError:
                raise TypeError(f"ERROR: {key} must be of type float or str.")
            except RuntimeError:
