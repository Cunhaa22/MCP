...
class Results:
    ...
    async def getSParameters(self, portA: int, portB: int, modeA: int=0, modeB: int=0,
                       runID: int=0):
        ...
        for key, value in portNumbers.items():
            ...
            try:
                portNumbers[key] = await self._portNumberProcessor(value)
    ...
    async def getFarField(
        ...
        try:
            freq = await self.__CheckParam.doCheck(freq)
        # If an exception occurs, handle it
        except TypeError:
    ...
    async def _portNumberProcessor(self, port: int):
