...
class CST_MicrowaveStudio:
    ...
    async def __init__(self, folder="", filename=""):
        ...
        # If a folder and filename are specified, call the __openFile method    
            ...
            await self.__openFile(folder, filename)
    ...
    async def __checkExtension(self, filename):
    ...
    async def __openFile(self, folder, filename):
        """Open a CST project.
        ...
        # Verify that the extension of file name is .cst
        self.__filename = await self.__checkExtension(filename)
        ...
        if projectExists:
            ...
            await self.__CST.OpenFile(fullPath)
            self.__MWS = self.__CST.Active3D()
    ...
    async def saveFile(self, folder="", filename="", includeResults=True):
