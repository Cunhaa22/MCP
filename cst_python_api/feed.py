...
async def create_coax_and_port(
    myCST: Any,
    *,
    xfeed: float = 0.0,
    yfeed: float = 0.0,
    ...
    await myCST.Build.Material.addMaterialFromLib(inner_mat)
    await myCST.Build.Material.addMaterialFromLib(consub_mat)
    await myCST.Build.Material.addMaterialFromLib(outer_mat)

    # Ensure the destination component exists before creating solids.
    await myCST.Build.Component.ensureExistence(coax_component)

    # Inner conductor (solid cylinder spanning the feed penetration depth).
    await myCST.Build.Shape.addCylinder(
        xMin=xfeed,
        yMin=yfeed,
        zMin=feed_z_bot,
        zMax=feed_z_top,
    ...
    if make_ground_cut:
        ...
        await myCST.Build.Boolean.subtract(
            f"{ground_component}",
            f"{coax_component}:{cut_name}"
        )

    await myCST.Solver.Port.addWaveguidePort(
