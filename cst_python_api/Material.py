...
materials_dir = Path(
    r"C:\Program Files (x86)\CST Studio Suite 2023\Library\Materials"
)
...
class Material:
    ...
    async def addNormalMaterial(self, name, eps, mu, colour, tanD = 0.0, sigma = 0.0, tanDM = 0.0, sigmaM = 0.0):
        ...
        for key, value in inputEMParams.items():
            ...
            try:
                inputEMParams[key] = await self.__CheckParam.doCheck(value)
    ...
    async def addAnisotropicMaterial(self, name, eps, mu, colour):
        """Adds normal material to the project.
    ...
    async def addMaterialFromLib(self, material_name: str) -> str:
        """
        Generate a VBA macro for a CST material by parsing its .mtd file.
    
        Parameters
        ----------
        material_name : str
            Base name of the material file (without .mtd).
        folder_path : str
            Directory path where the .mtd files are stored.
    
        Returns
        -------
        str
            A multi-line VBA macro string ready to be sent to CST.
    
        Raises
        ------
        FileNotFoundError
            If '<folder_path>/<material_name>.mtd' does not exist.
        ValueError
            If the file does not contain a [Definition] or [Type] section.
        """
        file_path = os.path.join(materials_dir, f"{material_name}.mtd")
        if not os.path.isfile(file_path):
            raise FileNotFoundError(
                f"No material file found for '{material_name}' in '{materials_dir}'"
            )
        ...
        # 1. Locate [Definition]
        try:
            start = next(i for i, ln in enumerate(lines) if ln.strip() == '[Definition]')
        except StopIteration:
            raise ValueError("No [Definition] section found in .mtd file.")
    
        # 2. Locate next section header after [Definition]
        end = len(lines)
        ...
        # 3. Parse each `.Key "val", "val2", ...` line
        ...
        for raw in lines[start+1:end]:
            ...
            # reconstruct argument list:  "v1", "v2", ...
            arg_list = ', '.join(f'"{v}"' for v in vals)
            # build the line
            attributes.append(f"    .{key} {arg_list}")
        ...
        # Send the VBA code to CST
        self.__MWS._FlagAsMethod("AddToHistory")
        result = self.__MWS.AddToHistory("define material: " + material_name, "\n".join(vba))
